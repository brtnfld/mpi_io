...
int main(int ac, char **av)
{
    ...
    //int64_t  buf_size = 4096LL;
    int64_t buf_size = 67108864;
    ...
    if(hdf5) {
      ...
      if( mpi_rank == 0) {
        ...
        H5Pset_alignment(fapl_id, 0, 2*1048576); 
      ...
      //ret = H5Pset_fapl_mpio(fapl_id, comm_self, MPI_INFO_NULL);
        ...
        if(strcmp(av[1],"-c")==0 || strcmp(av[1],"-t")==0) {
          for (i=0; i < num_vars; i++) {
            ...
            dcpl = H5Pcreate (H5P_DATASET_CREATE);
            hsize_t chunk_sz = buf_size_per_proc/2;
            ret = H5Pset_chunk (dcpl, 1, &chunk_sz);
            
            /* Create the dataset collectively */
            dset_id = H5Dcreate2(file_id, DATASETNAME[i], H5T_NATIVE_DOUBLE,
                                 file_space_id, H5P_DEFAULT, dcpl, H5P_DEFAULT);
    ...
    if(mpi_rank == 0) {
      ...
      printf("%s WRITE time is avg, min, max: %lf %f %f s.\n", hdftype, Sum_total_time/mpi_size, Min_total_time, Max_total_time);
