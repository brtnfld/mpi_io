...
int main(int ac, char **av)
{
    ...
    MPI_File fh;
    //   char *filename = "daos:mpitest.data";
    ...
    //MSB int64_t  buf_size = 5368709120LL;
...
//    buf_size = 5*1024*1024*1024;
    ...
    for (i=0; i < num_vars; i++) {
     ...
     //     != MPI_SUCCESS){
     if ((mpi_err = MPI_File_iwrite_at(fh, mpi_off, writedata, buf_size_per_proc, MPI_BYTE,&request[i]))
          != MPI_SUCCESS){
